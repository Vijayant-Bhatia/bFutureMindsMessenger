package com.barclaycard.hackathon.befuturemind.controller;

import com.barclaycard.hackathon.befuturemind.model.ActionResponse;
import com.barclaycard.hackathon.befuturemind.service.NPLService;
...
public class CallbackController {
    ...
    public ResponseEntity<Void> handleCallback(@RequestBody final String payload,
                                               @RequestHeader("X-Hub-Signature") final String signature, HttpSession session) {
    ...
    private TextMessageEventHandler newTextMessageEventHandler() {
        return event -> {
            ...
            try {
                ...
                ActionResponse actionResponse = nplService.getText(senderId, messageText);
                sendReadReceipt(senderId);
                sendTypingOn(senderId);
                if(!"goodbye.action".equals(actionResponse.getAction())) {
                    sendTextMessage(senderId, actionResponse.getResponse());
                }
                if(actionResponse.isServiceCallFlag()){
                    actionResponse.setResponse(messageText);
                    String text = nplService.hitServices(actionResponse);
                    if(null != text && !"".equals(text)) {
                        sendTextMessage(senderId, text);
                        sendTextMessage(senderId, "Do you want me to help you with something else?");
                    }
                }
                if("goodbye.action".equals(actionResponse.getAction()))
